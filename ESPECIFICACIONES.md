...
- la ventana inicial de actividad es de **30 minutos**; cuentan únicamente los commits de la rama y comentarios humanos útiles del Issue. `updated_at` del PR, actividad de bots, checks, Sonar, CodeRabbit, etiquetas y comandos de coordinación no cuentan;
- después de al menos 30 minutos sin actividad verificable, el barrido horario marca el Issue como `estado: requiere recuperación` y `/tomar` puede recuperarlo conservando la rama canónica y únicamente el PR abierto asociado a esa rama; la recuperación genera un UUID nuevo, reemplaza la metadata `Reserva: <UUID>` del PR e invalida el UUID anterior;
